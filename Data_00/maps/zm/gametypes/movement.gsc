...
Zombie_CheckAttack()
{
	...
	while(1)
	{
		if(self.attract == 0)
		{
			...
			{
				self.attacks = true;
				self maps\zm\gametypes\zombies::Zombie_Attack(self.goal);		
				wait 1;
				self.attacks = false;
			}
		}
...
getWaypoint(player)
{
	...
	if(self.myWaypoint == player || !isDefined(self.myWaypoint))
	{
		...
		if(counter > 0)
			...
			self thread maps\zm\gametypes\zombies::Zombie_Death(2,2);
	}
...
attackPlayer(target)
{
	...
	if(Distance(target.origin,self.origin) <= 60)
	{
		...
		self maps\zm\gametypes\zombies::Zombie_Attack(target);
		wait .8;
	}	
}
getTarget()
