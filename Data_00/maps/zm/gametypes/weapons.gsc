...
Zombie_Damage(iDamage, player, type)
{
	...
	else
	{
		...
		if(!isDefined(type))
			self thread maps\zm\gametypes\zombies::Zombie_Death(2);	
		//else if(self.type == "normal")
			self thread maps\zm\gametypes\zombies::Zombie_Death(2,undefined,undefined,"tesla");	
...
ApothiconServant(weapon,grenade)
{
	...
	foreach(zombie in level.zombie)
	{
		...
		if(isDefined(zombie.hitbox) && dist <= maxdist)
		{
			if(zombie.type != "dog")
...
ApothiconServant_old() // deprecated
{
	...
	foreach(zombie in level.zombie)
	{
		...
		if(isDefined(zombie.hitbox) && dist <= 500)
		{
...
Zombie_SlowDown(player)
{
	...
	{
		player.score += (50 * level.multiplier);
		self thread maps\zm\gametypes\zombies::Zombie_Death(3);
	}
	...
	*/
	...
	if(!self.paraslow)
	{
		...
		if(self.type == "normal")
			...
			self thread maps\zm\gametypes\zombies::Zombie_Animation("dog_idle");
	}
	wait 5;
	self.paraslow = false;
	//self.speed = self.speed * 20;
	if(self.type == "normal")
		...
		self thread maps\zm\gametypes\zombies::Zombie_Animation("dog_run");
}
ParaFly()
{
	self endon("disconnect");
...
Thunder_Kill(counter)
{
	timer = counter / 100;
	wait timer;
	self thread maps\zm\gametypes\zombies::Zombie_Death(4);
}
RaygunM2()
{
...
SpecialistHud()
{
	...
	self.specialisthud thread maps\zm\gametypes\utilities::destroy_on_end_game();
...
FlameDeath()
{
	...
	fire = spawnFx(level.fire_large, self.origin);
	triggerfx(fire);	
	self thread maps\zm\gametypes\zombies::Zombie_Animation("death");		
	wait 2.5;	
	self thread maps\zm\gametypes\zombies::Zombie_Death(2);		
	wait 2;
	fire delete();
