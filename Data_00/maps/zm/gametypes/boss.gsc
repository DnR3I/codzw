...
Dragon_Damage(weaponname)
{
	...
	while(1)
	{
		...
		{
			damage = self maps\zm\gametypes\zombies::doDamage(iDamage,attacker,victim,sWeapon,vPoint,killedwith);	
...
Nachtboss()
{
	...
	*/
	...
	foreach(zombie in level.zombie)
	{
		...
		{
			zombie thread maps\zm\gametypes\zombies::Zombie_Death(4);
			waitFrame();
		}
	}
	level notify("dragon_defeated");
