...
onPlayerConnected()
{
	...
	self thread maps\zm\gametypes\weapons::WonderWeapons();
...
onPlayerSpawned()
{
    ...
    for(;;)
    {
        ...
        // petit délai sinon HUD pas affiché
        wait 0.05;

        // applique modèle + icône
...
hud_health()
{
	...
	self.HP = level createServerFontString("objective", 1.2);
    self.HP setPoint("TOPRIGHT", "TOPRIGHT", -135, h); 
	self.HP setText("HP:");
	self.HP.color = (1,1,1);
	self.HP.glowalpha = 1;
	self.HP thread destroy_on_end_game();
	...
	self.healthnum.x = x + 83;
	self.healthnum.y = y - 12;
	self.healthnum.alpha = 1;
	self.healthnum.sort = 2;
	self.healthnum.fontscale = 1.2;
	self.healthnum.archived = 0;
	self.healthnum.color = ("1,1,1");
	self.healthnum thread destroy_on_end_game();
	...
	while(1)
	{
		...
		width = int(self.health/self.maxhealth*100);
		if(width <= 0)
			width = 1;
...
doLoadout()
{
	...
	if(getDvarInt("challenges") == 1)
		self thread Challengesystem();

	self thread maps\zm\gametypes\weapons::ParalyzerRecovery();
	...
	if(level.state == "ingame")
	{
		...
		endgame = self maps\zm\gametypes\zw::tryEndGame();		
		if(endgame)
			return;
		self thread teamFix();	
		level waittill("end");
...
setPlayerModel()
{
    ...
    // liste des persos déjà utilisés
    invalid = [];
    length = 0;
    ...
    // ====== RICHTOFEN ======
    if(self getValidPlayerModel("Richtofen", invalid))
    {
    ...
    // ====== NIKOLAI ======
    else if(self getValidPlayerModel("Nikolai", invalid))
    {
    ...
    // ====== DEMPSEY ======
    else if(self getValidPlayerModel("Dempsey", invalid))
    {
    ...
    // ====== TAKEO ======
    else if(self getValidPlayerModel("Takeo", invalid))
    {
...
revivePlayer(time) 
{
	...
	if(c > 1 && !self hasWeapon(level.startpistol)) lastweapon = false;
	else lastweapon = true;
	c = 0;
	foreach(primary in primaryWeapons) 
	{
		c++;
	}
	if(c > 2) lastweapon = false;
...
reviveCountDown() 
{
	...
	if(level.players.size == 1 && self.zombiePerks["Quick Revive"]) 
	{
...
revivebar(msg)
{
	...
	self.progresstext.foreground = 7;
...
ZombieHud()
{
	...
	zombiecounter thread destroy_on_end_game();

	while(1)
	{		
		if(!isDefined(level.zombies_left))
		...
		{			
			...
			zombiehud.alpha = 0;
			//self.hud_background2 fadeovertime(2);
			//self.hud_background2.alpha = 0;
		}
		else if(zombiecounter.alpha == 0)
		{
			zombiecounter fadeovertime(2);
		...
		if(level.zombies_left <= 0)
		{
			foreach(zombie in level.zombie)
			{
				...
				{
					zombie thread maps\zm\gametypes\zombies::Zombie_Death(1);
				}
			}
		}

		level waittill_any("zombie_died", "zombie_spawned", "start");
	}
}
...
MoneyHud()
{
    ...
    while(1)
    {
        y = 370; // position verticale de départ
		c = 385;
		i = 375;
        foreach(player in level.players)
        {
            // HUD Score
            if (!isDefined(level.cashhud_value[player.guid]))
            {
				playerNAME = level createServerFontString("objective", 0.8);
            	playerNAME setPoint("TOPLEFT", "TOPLEFT", 55, c); 
				playerNAME setText(player.name);
				playerNAME.color = (1,1,1);
				playerNAME.glowalpha = 1;
				playerNAME.glowcolor = player getColor(c);
                hud = level createServerFontString("objective", 1.3);
                hud setPoint("TOPLEFT", "TOPLEFT", 55, y);
                hud.foreground = 9;
                hud.color = (0.560, 0.941, 0.972);
                hud.glowalpha = 1;
                hud.glowcolor = (0.560, 0.941, 0.972);
                hud.hideWhenInMenu = true;

                hud.ownerGuid = player.guid;
                level.cashhud_value[player.guid] = hud;
            }

            // HUD Icone personnage
            if (!isDefined(level.cashhud_icon[player.guid]))
            {
                iconHud = NewHudElem();
                iconHud.x = -92; // position X (à gauche du texte argent)
                iconHud.y = i - 10; // centré verticalement
                iconHud.alignX = "left";
                iconHud.alignY = "top";
                iconHud.foreground = true;
                iconHud.sort = 9;
                iconHud.alpha = 1;

                // Choisir l’icône en fonction du personnage
                switch(player.character)
                {
                    case "Richtofen":
                        iconHud setShader("cardicon_gasmask", 30, 30);
                        break;
                    case "Nikolai":
                        iconHud setShader("cardicon_nightvision_2", 30, 30);
                        break;
                    case "Dempsey":
                        iconHud setShader("cardicon_nightvision_1", 30, 30);
                        break;
                    case "Takeo":
                        iconHud setShader("cardicon_sniperscope", 30, 30);
                        break;
                }

                iconHud.ownerGuid = player.guid;
                level.cashhud_icon[player.guid] = iconHud;
            }
...
getColor(c)
{
    switch(c)
    {
        case 0: case 6: return (1,0,0);   // Rouge
        case 1: case 7: return (0,1,0);   // Vert
        case 2: case 8: return (0,0,1);   // Bleu
        case 3: case 9: return (1,1,0);   // Jaune
        case 4: case 10: return (0,1,1);  // Cyan
        case 5: case 11: return (0,0,0);  // Noir
        default: return (1,1,1);          // Blanc
    }
}
...
Update_MoneyHud()
{
    self endon("death");
    self endon("disconnect");
    self endon("game_over"); // 🔥 ajout pour tuer le thread à la fin de la game

    for(;;)
    {
        self waittill("money");
        if (isDefined(level.cashhud_value[self.guid]))
...
{
	self endon("disconnect");
	//self waittill("loadout_complete");
	self waittill("intro_complete");
	level endon("game_over");

	oldweapon = self getCurrentWeapon();
	name = self createFontString("objective", 1);
	name setPoint( "RIGHT", "RIGHT", -118, 200 );
	name setText(getWeaponName(oldweapon));
	name.color = (43,255,255);
	name.foreground = 2;
	clip = self createFontString("objective", 3);
	clip setPoint( "RIGHT", "RIGHT", -142, 168 );
	clip.color = (43,255,255);
	clip.foreground = 2;
	stock = self createFontString("objective", 1.7);
	stock setPoint( "RIGHT", "RIGHT", -108, 175 );
	stock.color = (43,255,255);
	stock.foreground = 2;

	name thread destroy_on_end_game();
	stock thread destroy_on_end_game();
	clip thread destroy_on_end_game();
	...
	{		
		...
		if(oldweapon != weapon)
		{
			name setText(getWeaponName(weapon));
			oldweapon = weapon;
		}			
		...
		if(stockCount <= 0)
        {
            stock.color = (200/255,0,0);
        }
		else
		{
			stock.color = (1,1,1);
		}
		clip setValue(clipCount);
		stock setValue(stockCount);

		self waittill_any("spawned_player", "reload", "weapon_fired", "weapon_change", "maxammo", "newweapon", "specfade");
	}
}
		...
		if(self.isDown == 0 && self.protected == 0 && validspot)
		{
			...
			if(counter > max)
			{
				...
				if(BulletTracePassed(self.origin, self.origin+(0,0,1000), false, self) && spawnpos != (0,0,0))
				{
					...
					dragon.speed = 12;
					dragon thread maps\zm\gametypes\zw::Dragon_Wind();
				...
				else
				{
					...
					//thread maps\zm\gametypes\zw::locatePos(level.spawns_zombies[level.spawns_zombies.size]);
...
SpawnIntro()
{
	...
	wait 1.5;
	self show();
	Camera = spawn("script_model",self.origin+(0,0,5000));
	Camera setModel("tag_origin");
	Camera NotSolid();
	Camera EnableLinkTo();
	Camera RotateTo(self.angles + (90,0,0),.1);
	self PlayerLinkWeaponviewToDelta(Camera, "tag_player", 1.0, 0, 0, 0, 0, false);
	wait .1;
	Camera moveTo(self.origin,3,0,2);
	if(getDvarInt("night") == 1)
		self visionsetnakedforplayer("cobra_sunset3",1);
	else
		self visionsetnakedforplayer("default_mp",1);
	wait 1.6;	
	self visionsetnakedforplayer("coup_sunblind",1.5);
	Camera RotateTo(self.angles,1);
	wait 1.5;
	self ShellShock("ac130", 1.5);
	if(getDvarInt("night") == 1)
		self visionsetnakedforplayer("cobra_sunset3",2);
	else
...
Challengesystem()
{
	...
	self thread createChallenge();	
	self thread hideChallenge();
	self.challengehud = createRectangle("cardicon_abduction", "RIGHT", "RIGHT", 0,-210,230,200,(1,1,1), 20,1);
	self.challengehud thread destroy_on_end_game();
	while(true)
	{		
		self notify("newchallenge");
		self.challenge = self getChallenge();
		self.challengename setText(self.challenge.text);	
		self.challengename fadeovertime(1);
		self.challengeprogress fadeovertime(1);
		self.challengepercent fadeovertime(1);
		self.challengename.alpha = 1;
		self.challengeprogress.alpha = 1;
		self.challengepercent.alpha = 1;
		self waittill_any("newchallenge", "challenge_succeeded");
		type = self getReward();
		self.challengehud fadeovertime(3);
		self.challengehud.alpha = 0;
		self.challengename fadeovertime(1);
		self.challengeprogress fadeovertime(1);
		self.challengepercent fadeovertime(1);
		self.challengename.alpha = 0;
		self.challengeprogress.alpha = 0;
		self.challengepercent.alpha = 0;
		notifyData = spawnstruct();
		notifyData.iconName = "cardicon_cod4"; 
		notifyData.titleText = "Challenge succeeded"; 
		notifyData.notifyText = self.challenge.name; 
		if(type == "money")
		{
			notifyData.notifyText2 =  self.challenge.reward + " Cash retrieved"; 
			self.score += self.challenge.reward;
		}			
		else if(type == "perk")
		{
			notifyData.notifyText2 = "Random Perk achieved"; 
			self thread maps\zm\gametypes\zombies::GiveExtraPerk();
		}	
		else
		{
			notifyData.notifyText2 = "Random Gumball achieved"; 
			self thread maps\zm\gametypes\zombies::GiveExtraGumball();
		}
...
hideChallenge()
{
	...
	self.challengehud fadeovertime(1);
	self.challengehud.alpha = 0;
	self.challengename fadeovertime(1);
	self.challengeprogress fadeovertime(1);
	self.challengepercent fadeovertime(1);
...
createChallenge()
{
	...
	self.challengename = self createFontString( "Objective", 1.4 );
	self.challengename setPoint( "RIGHT", "RIGHT", -40, -205 );
	self.challengename.hideWhenInMenu = true;
	self.challengename.glowalpha = .4;
	self.challengename.glowcolor = (1,0,0);
	self.challengename.alpha = 0;
	//self.challengename setText(self.challenge.text);	

	self.challengepercent = self createFontString( "Objective", 1.4 );
	self.challengepercent setPoint( "RIGHT", "RIGHT", -100, -185 );	
	self.challengepercent.hideWhenInMenu = true;
	self.challengepercent.glowalpha = .4;
	self.challengepercent.glowcolor = (1,0,0);
	self.challengepercent.alpha = 0;
	self.challengepercent.foreground = 1;
	self.challengepercent setText("Percent:");
	self.challengepercent thread destroy_on_end_game();
	
	self.challengeprogress = self createFontString( "Objective", 1.4 );
	self.challengeprogress setPoint( "RIGHT", "RIGHT", -40, -185 );	
	self.challengeprogress.hideWhenInMenu = true;
	self.challengeprogress.glowalpha = .4;
	self.challengeprogress.glowcolor = (1,0,0);
	self.challengeprogress.alpha = 0;
	self.challengeprogress.foreground = 1;	
	self.challengeprogress thread destroy_on_end_game();
	for(;;)
	{		
		self.value = self.challenge.progress/self.challenge.amount * 100;
		//iprintln(self.value);
		float = reduceString(self.value);
		self.challengeprogress setValue(float);
		wait 1;
	}
}
reduceString(float)
...
{
	self notify("challenge_denied");
	if(isDefined(challenge))
		self.challenge = challenge;
	else
	{		
		challenge = level.challenge[randomInt(level.challenge.size)];
		self.challenge = challenge;
	}
	construct = spawnstruct(); 
	construct.name = challenge;
	construct.progress = 0;

	if(challenge == "Kill Zombies")
	{
		construct.amount = 10;
		construct.amount = (construct.amount * (randomInt(27)+3));
		construct.reward = construct.amount * 20;
		construct.text = "Kill " + construct.amount + " Zombies!";
		construct thread destroy_on_end_game();
		self thread KillZombies(construct.amount);
	}
	else if(challenge == "Kill Zombies with Melee")
	{
		construct.amount = randomInt(17);
		construct.amount += 3;
		if(level.round < 5)
			construct.reward = construct.amount * 200;
		else
			construct.reward = construct.amount * 400;
		construct.text = "Melee " + construct.amount + " Zombies!";
		construct thread destroy_on_end_game();
		self thread MeleeKillZombies(construct.amount);
	}
	else if(challenge == "Do Damage")
	{
		construct.amount = 10000;
		construct.amount *= (1 + randomInt(4));
		construct.reward = construct.amount / 25;
		construct.text = "Do " + construct.amount + " Damage!";
		construct thread destroy_on_end_game();
		self thread doDamage(construct.amount);
	}
	else if(challenge == "Protect The Area")
	{
		construct.amount = 30 * (1 + randomInt(3));
		construct.reward = construct.amount * 30;
		construct.text = "Protect the area[" + construct.amount + "s]!";
		construct thread destroy_on_end_game();
		self thread ProtectTheArea(construct.amount);
	}
	else if(challenge == "Survive Rounds")
	{
		construct.amount = (randomInt(4) + 1);
		construct.reward = construct.amount * 700;
		construct.text = "Survive " + construct.amount + " rounds!";
		construct thread destroy_on_end_game();
		self thread SurviveRounds(construct.amount);
	}
	else if(challenge == "Upgrade your Weapon")
	{
		construct.amount = 1;
		construct.reward = 3000;
		construct.text = challenge;
		construct thread destroy_on_end_game();
	...
	else
	{
		construct.amount = 0;
		construct.reward = 0;
		construct.text = "^1ERROR: Unknown Challenge";
	}
	return construct;
}
KillZombies(amount)
{
	self endon("disconnect");
	self endon("game_over");
	self endon("challenge_denied");
	self.mykills = 0;
	while(self.challenge.progress < amount)
	{
		wait .5;
		self.challenge.progress = self.mykills;
	}
	self notify("challenge_succeeded");
}
MeleeKillZombies(amount)
{
	self endon("disconnect");
	self endon("game_over");
	self endon("challenge_denied");
	...
	{
		wait .5;
		self.challenge.progress = self.meleekills;
	}
	self notify("challenge_succeeded");
}
doDamage(amount)
{
	...
	{
		wait .5;
		self.challenge.progress = self.damage_dealed;
	}
	self notify("challenge_succeeded");
}
ProtectTheArea(amount)
{
	self endon("disconnect");
	self endon("game_over");
	self endon("challenge_denied");
	trace = BulletTrace(self.origin,self.origin+(0,0,-1000), true, undefined );
	pos = trace["position"];
	for(i = 0;i < amount;i++)
	{
	    self.challenge.progress = i;
	    wait 1;
		if(Distance(self.origin, pos) > 300)
		{
		    i -= 2;
			if(i < 0)
			   i = 0;
		}
		else if(self.isDown == 1)
		{
		    i -= 2;
		}
		else if(self.anticampcounter > 0)
			self.anticampcounter--;
	}
	self notify("challenge_succeeded");
}
SurviveRounds(amount)
{
	self endon("disconnect");
	self endon("game_over");
	self endon("challenge_denied");
...
{
	zombie thread maps\zm\gametypes\weapons::Zombie_Damage(99999, self, "MOD_EXPLOSIVE");
	zombie.attract = 1;
...
grenadeBlow(grenade)
{
	...
	foreach(zombie in level.zombie)
	{
		...
		{
			zombie thread maps\zm\gametypes\weapons::Zombie_Damage(300,self,"MOD_EXPLOSIVE");
...
ChopperGunner()
{
	...
	self unlink();
	self.isInChopper = false;
	self thread maps\zm\gametypes\zw::NoFallDamage();
	...
	while(true)
	{
		...
		if(i > 0 && randomInt(3) == 2)
		{
			...
			foreach(zombie in level.zombie)
			{
				...
				{
					zombie thread maps\zm\gametypes\weapons::Zombie_Damage(1500, self, "MOD_EXPLOSIVE");
					waitFrame();
				}
			}
