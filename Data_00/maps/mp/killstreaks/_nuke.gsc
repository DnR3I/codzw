...
// =========================================================
//
// unused & unfinished file, maybe used in future updates, contains a nuke ending outro
//
// =========================================================

NukeCinematic()
{
	airplane = spawn("script_model", (10000,10000,3000));
	airplane setModel("vehicle_ac130_low_mp");
	airplane moveTo((-10000,-10000,3000), 20);
	movetoLoc = VectorToAngles((-10000,-10000,3000) - airplane.origin );	
	airplane RotateTo((0,movetoLoc[1],0), 0.1);
	wait 1;
	Camera = spawn("script_model", (10000,10000,3500));
	Camera setModel("tag_origin");
	Camera.angles = (0,movetoLoc[1],0); 
	Camera moveTo((0,0,3500), 10);	
	you = undefined;
	foreach(player in level.players)
	{
		...
		player PlayerLinkWeaponviewToDelta(Camera, "tag_player", 1.0, 0, 0, 0, 0, false);
		you = player;
	}
	wait 7;
	bomb = spawn("script_model", airplane.origin);
	bomb setModel(getweaponmodel("remotemissile_projectile_mp"));
	trace = bulletTrace(Camera.origin,Camera.origin+(0,0,-5000), false);
	bomb moveTo(trace["position"],3.5);
	Camera RotateTo(Camera.angles + (90,0,0),2);
	//Camera moveTo(trace["position"],3.5);	
	Camera LinkTo(bomb);
	wait 5;
	airplane delete();
}
