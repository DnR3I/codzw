...
init()
{
	...
	level.killstreakFuncs["ac130_ai"] = ::tryUseAC130AI;
	
	level.ac130Queue = [];
}


tryUseAC130AI( lifeId )
{
	...
	thread setAC130PlayerAI( self );
	level.total_vehicles++;
	return true;
}

setAC130PlayerAI( player )
{
	...
	level.ac130 thread startAC130AI( player );
}

startAC130AI(owner)
{
	owner endon("ac130player_removed");
	...
	for(;;)
	{
		...
		if(i > 0)
		{
			...
			MagicBullet("ac130_105mm_mp", self.planemodel.origin, target.origin, owner);
		}
...
removeAC130Player( player, disconnected )
{
	...
	if ( !disconnected && !level.ac130.isAI )
	{
...
thermalVision()
{
	...
	self visionSetThermalForPlayer("cobrapilot",1);
	...
	for (;;)
	{
		...
		else
		{
			...
			self visionSetThermalForPlayer("cobrapilot",.51);
			if ( isdefined( level.HUDItem[ "thermal_mode" ] ) )
				level.HUDItem[ "thermal_mode" ] settext ( &"AC130_HUD_THERMAL_WHOT" );
		}
