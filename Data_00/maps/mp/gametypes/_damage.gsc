...
{	
	self notify("laststand");
	counter = 0;
	/*self.oldweapons = [];
	primaryWeapons = self getWeaponsListPrimaries();
	foreach(primary in primaryWeapons)
	{		
		self.oldweapons[counter] = spawnstruct();
		self.oldweapons[counter].name = primary;
		self.oldweapons[counter].clip = self GetWeaponAmmoClip(self getCurrentWeapon());
		self.oldweapons[counter].stock = self GetWeaponAmmoStock(self getCurrentWeapon());
		counter++;
	}
	*/
	...
	if ( self _hasPerk( "specialty_finalstand" ) )
	{
		...
		self thread lastStandTimer( 40, true );		
	}
	/*
	...
	*/
	...
	else // normal last stand
	{
		...
		if(amount < 2 || self hasWeapon(getDvar("start_weapon")))
			lastweapon = true;
		else
			lastweapon = false;
		if ( !isDefined( pistolWeapon ) )
		{
			pistolWeapon = getDvar("start_weapon");
		...
		time = getDvarInt("bleedout_time");
		self thread lastStandTimer(time, false,lastweapon );
		self thread dieAfterTime(time);
	}
}
...
lastStandTimer( delay, isFinalStand,lastweapon )
{
	self endon( "death" );
	self endon( "disconnect" );
	self endon( "revive");
	...
	solo_withquick = false;
	multi_withquick = false;
	if(self.quickrevive && level.players.size == 1)
		solo_withquick = true;
	if(self.quickrevive && level.players.size > 1)
		multi_withquick = true;
	if(!solo_withquick || multi_withquick)
	{
		self thread maps\zm\gametypes\player::revivePlayer(delay);
		if(level.dieHardMode == 1 && level.dieHardMode != 2)
		{
			...
			{
				wait( delay -2 );
				self playlocalsound("UK_1mc_captureflag");
				wait 2;
	...
	else if(self.quickrevive && level.players.size == 1)
	{
		self thread maps\zm\gametypes\player::revivePlayer(getDvarInt("selfmediation_time"));
		wait getDvarInt("selfmediation_time");
		self notify("revived");
		self setMoveSpeedScale(1);
		self.lastStand = undefined;
		self.maxHealth = 100;	
		self thread canGetAttackedAgain();
		self EnableWeaponSwitch();
		if(!lastweapon)
			self takeWeapon(getDvar("start_weapon"));
		self switchToWeapon(current);	
		self thread takeInvalidWeapons();
		//self thread maps\zm\gametypes\player::Grenadesystem(true);
		self thread maps\mp\gametypes\_weapons::updateMoveSpeedScale( "primary" ); // one of these threads kills this thread
		self thread maps\mp\gametypes\_playerlogic::lastStandRespawnPlayer();	
		self notify( "revive");
	}
}
canGetAttackedAgain()
...
takeInvalidWeapons()
{
	...
	foreach(primary in primaryWeapons)
	{
		if(!maps\zm\gametypes\utilities::isValidWeapon(primary) && primary != "frag_grenade_mp")
