...
lastStandRespawnPlayer()
{
    ...
    */
    ...
    if (self isBot())
        self.maxHealth = getDvarInt("bot_health");
    else
        self.maxHealth = 100;
...
spawnPlayer()
{
    ...
    if (!gameFlag("prematch_done") || !hadSpawned && game["state"] == "playing")
    {
        ...
        {
            thread maps\mp\gametypes\_hud_message::oldNotifyMessage(
                game["strings"]["overtime"],
        ...
        {
            thread maps\mp\gametypes\_hud_message::oldNotifyMessage(
                game["strings"][team + "_name"],
                undefined,
                game["icons"][team] + "_blue",
                game["colors"]["blue"]
            );
        }
...
Callback_PlayerConnect()
{
    ...
    if (matchMakingGame() && game["clientid"] <= 24 && game["clientid"] != getMatchData("playerCount"))
    {
        ...
        assert(
            getdvarint("scr_runlevelandquit") == 1
            || (level.teamBased && (self.sessionteam == "allies" || self.sessionteam == "axis"))
            || (!level.teamBased && self.sessionteam == "none")
        );
...
setRestXPGoal()
{
    ...
    println(
        "Player " + self.name
        + " has rested for " + hoursSinceLastGame + " hours; gained "
        + restLevels + " levels of rest XP" + cappedString
        + ". Now has " + (restXPGoal - experience) + " rest XP; was " + (oldRestXPGoal - experience)
    );
